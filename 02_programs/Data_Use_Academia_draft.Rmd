...
models <- list(
  ...
  '(4)'=feols(log(data_papers) ~ log(NY.GDP.PCAP.PP.KD) + log(SP.POP.TOTL) , data=reg_df_fixedn, se='hetero'),
  '(5)'=feols(log(data_papers) ~ SPI.INDEX + log(NY.GDP.PCAP.PP.KD) + log(SP.POP.TOTL) , data=reg_df_fixedn, se='hetero'),
  #'(6)'=lm_robust(log(data_papers) ~ SPI.INDEX + SPI.INDEX.PIL2 + SPI.INDEX.PIL3 + SPI.INDEX.PIL4 + SPI.INDEX.PIL5 , data=reg_df_fixedn) ,  
  '(6)'=feols(log(data_papers) ~ SPI.INDEX.PIL1 + SPI.INDEX.PIL2 + SPI.INDEX.PIL3 + SPI.INDEX.PIL4 + SPI.INDEX.PIL5 + log(NY.GDP.PCAP.PP.KD) + log(SP.POP.TOTL)  , data=reg_df_fixedn, se='hetero')
...
df_plot <- country_scores_annual_df %>%
  ...
  select(iso3c, date, region, income_level, data_papers_3yr, SPI.INDEX.PIL4.5_3yr,SP.POP.TOTL,NY.GDP.PCAP.PP.KD,
         ...
         NY.GDP.PCAP.PP.KD_3yr, SP.POP.TOTL_3yr,qual_papers_3yr)


# Log data papers population and GDPpc regression adjusted
# mod <- fixest::feols(log(data_papers_3yr)~  log(SP.POP.TOTL) + log(NY.GDP.PCAP.PP.KD)  | date,  data=country_scores_annual_df) 
...
median_y <- median(df_plot$data_papers_3yr, na.rm = TRUE)
...
# For labels 
min_y <- min(df_plot_outl$data_papers_3yr, na.rm = T)
max_y <- max(df_plot_outl$data_papers_3yr, na.rm = T)
...
# ggplot(df_plot, aes(x = SPI.INDEX.PIL4_3yr, y = data_papers_3yr)) +
...
#   labs(y = "# papers using data (avg. 2017-2019)", 
...
# Scatter plot - qual papers - region
# ggplot(df_plot, aes(x = SPI.INDEX.PIL4_3yr, y = data_use_reg_adjusted_3yr_v2)) +
#   geom_point(aes(colour = region)) +
#   geom_text_repel(aes(label = iso3c)) +
#   geom_vline(xintercept = median_x_v2, linetype="dashed", color = "red") +
...
#   theme(legend.position = "bottom", legend.title = element_blank())  + 
#   labs(caption = "Red lines indicate median values.")
# path <- paste0(dirname(getwd()),"/02_programs/figures/country_classification_desertsoceans_region_v2.png")
# ggsave(path, width = 9, height = 8.1)
...
df_plot <- df_plot %>%
  mutate(category = ifelse(SPI.INDEX.PIL4_3yr <= median_x & data_papers_3yr <= median_y, "Desert",
                           ifelse(SPI.INDEX.PIL4_3yr > median_x & data_papers_3yr <= median_y, "Swamp",
                                  ifelse(SPI.INDEX.PIL4_3yr <= median_x & data_papers_3yr > median_y_v2, "Oasis",
                                         "Lake")))) %>%
  mutate(category_adj = ifelse(SPI.INDEX.PIL4_3yr <= median_x & data_use_reg_adjusted_3yr_v2 <= median_y_v2, "Desert",
                           ifelse(SPI.INDEX.PIL4_3yr > median_x & data_use_reg_adjusted_3yr_v2 <= median_y_v2, "Swamp",
                                  ifelse(SPI.INDEX.PIL4_3yr <= median_x & data_use_reg_adjusted_3yr_v2 > median_y_v2, "Oasis",
                                         "Lake")))) %>%
mutate(category_adj_both = ifelse(SPI.INDEX.PIL4_3yr_reg_adjusted_3yr_v2 <= median_x_v2 & data_use_reg_adjusted_3yr_v2 <= median_y_v2, "Desert",
                           ifelse(SPI.INDEX.PIL4_3yr_reg_adjusted_3yr_v2 > median_x_v2 & data_use_reg_adjusted_3yr_v2 <= median_y_v2, "Swamp",
                                  ifelse(SPI.INDEX.PIL4_3yr_reg_adjusted_3yr_v2 <= median_x_v2 & data_use_reg_adjusted_3yr_v2 > median_y_v2, "Oasis",
                                         "Lake")))) %>%  
...
Fig. Classifications based on Unadjusted # of Papers (avg. 2017-2019)
...
#map of deserts oceans
 #set color pallete
  ...
  names(col_pal) <- c("Lake","Oasis","Swamp","Desert" ) 
   ...
   p1 <- ggplot() +
    geom_map(data = df_plot, aes(map_id = iso3c, fill = category), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
...
Fig. Classifications based on Regression adjusted # papers using data (avg. 2017-2019)
...
#map of deserts oceans
 #set color pallete
  ...
  names(col_pal) <- c("Lake","Oasis","Swamp","Desert" ) 
   ...
   p1 <- ggplot() +
    geom_map(data = df_plot, aes(map_id = iso3c, fill = category_adj), map = maps$countries) + 
    geom_polygon(data = maps$disputed, aes(long, lat, group = group, map_id = id), fill = "grey80") + 
    geom_polygon(data = maps$lakes, aes(long, lat, group = group), fill = "white")  +
     geom_path(data = maps$boundaries,
               aes(long, lat, group = group),
