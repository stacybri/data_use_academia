...
#a set of duplicate articles were double coded.  Identify these
duplicates <- duplicated((mturk_articles_df %>% select(abstract, title)))

mturk_double_code_df <- mturk_articles_df %>%
  filter(duplicates)

mturk_articles_df <- mturk_articles_df %>%
  filter(!duplicates) #keep just one code per article, so removing

#count of rows
duplicates_num <- nrow(mturk_double_code_df)
mturk_num <- nrow(mturk_articles_df)
...
mturk_double_code_df2 <- mturk_double_code_df %>%
  mutate(
...
#get list of double coded articles
mturk_dbl_article_list <- mturk_double_code_df2$title

#get list of reference words for regular expressions
...
#get data with first coder
mturk_first_coded_df <- mturk_articles_df %>%
  filter(title %in% mturk_dbl_article_list) %>%
  select(paper_id, title, abstract, Answers.country, mag_field_of_study,doi, venue, journal, Answers.data_type, Answers.data_use, WorkerId, WorkTimeInSeconds ) %>%
  rename(First.country=Answers.country,
         First.data_type=Answers.data_type,
         First.data_use=Answers.data_use,
         First.WorkerId=WorkerId,
         First.WorkTimeInSeconds=WorkTimeInSeconds)

#worker exclude list.
#exclude workers with more than 3 matches and with less than 33% match rate
workers <- c('A1IFIK8J49WBER', 'A299J4PKHAEU9H','A1IHI23KH87K5W','A3HNEYFOIJWPH1',
             'A2KLJKDG90K1PP','AV22FQTJNBUZT','A2YYQJ3JWA2KGD')

#shape data for second coder
mturk_second_coded_df <- mturk_double_code_df2 %>%
  select(paper_id, title, abstract, Answers.country, mag_field_of_study,doi, venue, journal, Answers.data_type, Answers.data_use, WorkerId, WorkTimeInSeconds) %>%
  rename(Second.country=Answers.country,
         Second.data_type=Answers.data_type,
         Second.data_use=Answers.data_use,
         Second.WorkerId=WorkerId,
         Second.WorkTimeInSeconds=WorkTimeInSeconds)

mturk_irr_df <- mturk_first_coded_df %>%
  left_join(mturk_second_coded_df) %>%
  # filter(!(First.WorkerId %in% workers)) %>% #explore removing low performing workers
  # filter(!(Second.WorkerId %in% workers)) %>%
  mutate(data_use_match=First.data_use==Second.data_use,
         country_match=First.country==Second.country,
         data_use_naive=First.data_use=="Yes",
         data_use_regrex=if_else(grepl(reference_word_list_4, abstract),"Yes","No"),
         data_use_match_regex=Second.data_use==data_use_regrex) %>%
  select(paper_id, title, abstract, data_use_match,data_use_naive,First.data_use,Second.data_use,First.WorkTimeInSeconds,Second.WorkTimeInSeconds,First.data_type,Second.data_type, country_match, everything() ) %>%
  filter()
 
# irr_stats <- mturk_irr_df %>%
#   group_by(First.data_type) %>%
#   summarise(
#     data_use_match=mean(data_use_match),
#     
#     country_match=mean(country_match),
#     n=n()
#   ) %>%
#   arrange(-n)
# 
# regex_stats <- mturk_irr_df %>%
#   group_by(data_use_naive) %>%
#   summarise(
#     data_use_match=mean(data_use_match),
#     
#     country_match=mean(country_match),
...
#   arrange(-n)
# 
# #get dataset with regex trues
# mturk_irr_regex_true_df <- mturk_irr_df %>%
#   filter(data_use_regrex=='No')
# 
# worker_stats1 <- mturk_irr_df %>%
#   group_by(First.WorkerId) %>%
#   summarise(
#     data_use_match=mean(data_use_match),
#     
#     country_match=mean(country_match),
...
#   arrange(-n)
# 
# worker_stats2 <- mturk_irr_df %>%
#   group_by(Second.WorkerId) %>%
#   summarise(
#     data_use_match=mean(data_use_match),
#     
#     country_match=mean(country_match),
...
duplicates_num <- nrow(mturk_irr_df)

mean(mturk_irr_df$data_use_match)
mean(mturk_irr_df$data_use_naive)
mean(mturk_irr_df$data_use_regrex=="Yes")
mean(mturk_irr_df$First.data_use=="Yes")
mean(mturk_irr_df$Second.data_use=="Yes")

mean(mturk_irr_df$data_use_match_regex)
mean(mturk_irr_df$country_match)

#confusion matrix
#Creating confusion matrix
confusion_mat <- confusionMatrix(data=factor(mturk_irr_df$First.data_use,levels=c("Yes","No")), reference = factor(mturk_irr_df$Second.data_use,levels=c("Yes","No")))
 
#Display results 
confusion_mat

# #Creating confusion matrix among regex trues
# confusion_mat <- confusionMatrix(data=factor(mturk_irr_regex_true_df$First.data_use,levels=c("Yes","No")), reference = factor(mturk_irr_regex_true_df$Second.data_use,levels=c("Yes","No")))
#  
# #Display results 
# confusion_mat
# 
# # #now do for regex
# confusion_mat_regex <- confusionMatrix(data=factor(mturk_irr_df$First.data_use,levels=c("Yes","No")), reference = factor(mturk_irr_df$data_use_regrex,levels=c("Yes","No")))
# 
# confusion_mat_regex
...
```{r eval=FALSE, include=FALSE}
#look at how rankings compare between double coded
mturk_irr_rankings_df1 <- mturk_irr_df %>%
  filter(First.data_use=="Yes")

mturk_irr_rankings_df2 <- mturk_irr_df %>%
  filter(Second.data_use=="Yes")

mturk_irr_rankings_df1 <- cbind( sapply(country_scores_df$country, function(x) as.integer(grepl(x, mturk_irr_rankings_df1$First.country))))
mturk_irr_rankings_df2 <- cbind( sapply(country_scores_df$country, function(x) as.integer(grepl(x, mturk_irr_rankings_df2$Second.country))))

#collapse to country level
mturk_irr_country_rankings_df1 <- mturk_irr_rankings_df1 %>%
  as_tibble() %>%
...
mturk_irr_country_rankings_df2 <- mturk_irr_rankings_df2 %>%
  as_tibble() %>%
  select_if(is.numeric) %>%
  map_dbl(sum) %>%
  stack() %>%
...
mturk_country_rankings <- mturk_irr_country_rankings_df1 %>%
  left_join(mturk_irr_country_rankings_df2) %>%
  filter(first.count+second.count>0)

cor(mturk_country_rankings$first.count, mturk_country_rankings$second.count)

ggplot(mturk_country_rankings, aes(x=first.count, y=second.count)) +
  geom_point() +
  geom_smooth(method = "lm") +
  geom_richtext(
    aes(x = 1, y = 6,label = eq_plot_txt(mturk_country_rankings, second.count, first.count), hjust=0.2)
  ) +
  geom_text_repel(aes(label=country)) +
  bbc_style()
...
# One concern may be that the Amazon Mechanical Turk workers were unreliable and gave inaccurate answers.  As a test of this, the team double coded
# `r duplicates_num` articles by Mechanical Turk workers. Using this data, inter-rater reliability can be assessed.
# 
# Reliability is around 78% agreement for countries and a little under 60% agreement for whether the article uses data.  This indicates that classifying whether an article uses data is challenging even for human raters. 
...
anim <- country_scores_annual_df %>%
  group_by(year) %>%
  mutate(rank=rank(-data_papers_estimate)) %>%
  arrange(desc(data_papers_estimate)) %>%
  filter(row_number()<=10) %>%
ggplot( aes(x=rank,group=country, fill=region)) +
  geom_col(aes(y=data_papers_estimate,
            height=data_papers_estimate)) +
  geom_text(aes(y = 0, label = paste(country, " ")), vjust = 0.2, hjust = 1) +
  geom_text(aes(y=data_papers_estimate,label = scales::comma(round(data_papers_estimate,0)), hjust=0)) +  
  ...
  labs(title = 'Academic Articles : {closest_state}',
       subtitle  =  "Top 10 Countries",
       caption  = "Academic Article | Data Source: World Bank Data") +
...
<!-- ## Comparison of Articles Published and Number of Poverty Surveys -->

<!-- The figure below shows the relationship between the number of poverty surveys fielded in a county and the number of academic publications.  The number of poverty surveys in each country comes from the World Bank's Poverty and Inequality Platform (PIP) and includes all nationally representative surveys from 2001 to 2020, the same span of years as the count of academic articles.  Regression estimates indicate an elasticity of 0.7 between the number of poverty surveys and academic articles using data, that is a 1% increase in the number of poverty surveys increases the number of academic articles by 0.7%.  -->

<!-- Figure. Relationship Between Articles and Number of Poverty Surveys -->
<!-- ```{r householdsurveys} -->
...
<!--          iso3c=country_code) %>%  -->
<!--   filter(between(indicator_date,2001,2020))  -->

<!-- pip_df <- pip_df %>% -->
<!--   left_join(country_metadata) %>% -->
<!--   group_by(iso3c) %>% -->
<!--   summarise(poverty_surveys=n())  -->
...
<!-- pip_df  %>% -->
<!--   ggplot( aes(y=data_papers, x=poverty_surveys)) + -->
<!--     geom_point() + -->
<!--     geom_text_repel(aes(label=iso3c)) + -->
<!--     scale_x_log10(labels=scales::comma) + -->
<!--     scale_y_log10(labels=scales::comma) + -->
<!--     xlab("Number of Poverty Surveys ") + -->
<!--     ylab('Number of Papers using Data') + -->
<!--     geom_smooth(method = "lm") + -->
<!--       geom_richtext( -->
<!--     aes(x = 10, y = 20,label = eq_plot_txt(pip_df, log(data_papers),log(poverty_surveys))), hjust=0.2, size=4 -->
<!--     ) + -->
<!--     bbc_style() + -->
<!--     labs(      subtitle="Data from 2001-2020" -->
<!--     ) -->
