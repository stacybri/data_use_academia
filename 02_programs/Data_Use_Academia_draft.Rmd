...
models <- list(
  ...
  #'(6)'=lm_robust(log(data_papers) ~ SPI.INDEX + SPI.INDEX.PIL2 + SPI.INDEX.PIL3 + SPI.INDEX.PIL4 + SPI.INDEX.PIL5 , data=reg_df_fixedn) ,  
  '(5)'=lm_robust(log(qual_papers) ~  SPI.INDEX.PIL4  + log(NY.GDP.PCAP.PP.KD) + log(SP.POP.TOTL) , data=(reg_df_fixedn %>% filter(qual_papers>0)))
  # '(6)'=lm_robust(log(data_papers) ~ SPI.D4.1.1.POPU + SPI.D4.1.2.AGRI + SPI.D4.1.3.BIZZ + SPI.D4.1.4.HOUS + SPI.D4.1.5.AGSVY + SPI.D4.1.6.LABR + SPI.D4.1.7.HLTH + SPI.D4.1.8.BZSVY +  SPI.D4.2.3.CRVS + SPI.D4.3.GEO.first.admin.level + log(NY.GDP.PCAP.PP.KD) + log(SP.POP.TOTL) , data=reg_df_fixedn) 
  )
...
df_plot <- df_plot %>%
  mutate(category = ifelse(SPI.INDEX.PIL4_3yr <= median_x & data_papers_pcap_3yr <= median_y, "Desert",
                           ifelse(SPI.INDEX.PIL4_3yr > median_x & data_papers_pcap_3yr <= median_y, "Swamp",
                                  ifelse(SPI.INDEX.PIL4_3yr <= median_x & data_papers_pcap_3yr > median_y_v2, "Oasis",
                                         "Lake")))) %>%
...
```{r}
breakdown1 <- df_plot %>% 
  group_by(category_adj, region) %>%
  summarise(n=n()) %>%
  ungroup() %>%
  group_by(category_adj) %>%
...
breakdown3 <- df_plot %>% 
  group_by(category_adj, income_level) %>%
  summarise(n=n()) %>%
  ungroup() %>%
  group_by(income_level) %>%
  mutate(share=n/sum(n))
...
Fig. Classifications based on # of Papers per capita (avg. 2017-2019)
...
ggally2_df <- data_use_df %>%
  ...
  filter(!( is.infinite(data_use_pcap_3yr) | is.infinite(data_use_reg_adjusted_3yr) |  is.infinite(data_use_ratio_3yr))) 

GGally::ggpairs(ggally2_df,
