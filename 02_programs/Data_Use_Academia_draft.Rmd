...
Below we show the countries that over or underperform in terms of their GDP per capita in 2020 compared to the number of papers using data per million persons.  This is a way to check for "outliers" in terms of log per capita academic output compared to log per capita GDP.  This analysis is similar to that done in @das2013us.
...
mod <- lm(log(data_papers_pcap) ~ log(NY.GDP.PCAP.PP.KD),  data=gdp_df_2019) 
#summary(mod)
...
gdp_overperformers_plot <- gdp_overperformers_plot %>%
  ...
  ggtitle("Over/Under Perfomers - GDP per capita"
          )
...
```{r}
p1 <- ggplot(gdp_df_2019, aes(y=data_papers_pcap, x=NY.GDP.PCAP.PP.KD)) +
  geom_point() +
  geom_text_repel(aes(label=iso3c)) +
  scale_x_log10(labels=scales::comma) +
  scale_y_log10(labels=scales::comma) +
